const colors = [
  'rgba(255, 100, 255, 0.8)',  // Light Pink
  'rgba(200, 100, 255, 0.8)',  // Light Purple
  'rgba(150, 100, 255, 0.8)',  // Medium Purple
  'rgba(100, 150, 255, 0.8)',  // Light Blue
  'rgba(150, 255, 255, 0.8)',  // Cyan
  'rgba(255, 150, 200, 0.8)'   // Rose
];
...
function createParticle(x, y, size, color) {
  ...
  const randomSize = size + (Math.random() * 15 - 7.5);
  ...
  particle.style.boxShadow = `0 0 ${randomSize}px ${color}`;
...
setInterval(() => {
  if (particleCount < maxParticles && lastX !== 0 && lastY !== 0) {
      createParticleBurst(lastX, lastY);
  }
}, 100);
